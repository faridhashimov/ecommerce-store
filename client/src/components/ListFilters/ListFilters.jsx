  ...
  return (
    <FilterContainer d={d}>
              ...
              >
                <FilterName
                  style={{
                    color: !cat
                      ? '#1b1b1b'
                      : Object.entries(cat).flat().includes(item)
                        ? '#f27a1a'
                        : '#1b1b1b',
                  }}
                >
      ...
      <CategoriesContainer>
        ...
        <Categories
          height={
            openFilter.includes('gender') && filters.genderFilter.length >= 4
              ? '120px'
              : openFilter.includes('gender') && filters.genderFilter.length < 4
                ? 'fit-content'
                : '0px'
        ...
        >
          <Filters>
            {filters.genderFilter.map((item) => (
              ...
              >
                {!cat ? (
                  <StyledCheckbox c="#999" component={CheckBoxOutlineBlank} />
                ) : Object.entries(cat).flat().includes(item) ? (
                  <StyledCheckbox c="#F27A1A" component={CheckBox} />
                ) : (
                  <StyledCheckbox c="#999" component={CheckBoxOutlineBlank} />
                )}
      ...
      <CategoriesContainer>
        ...
        <Categories
          height={
            openFilter.includes('brand') && filters.brandFilter.length >= 10
              ? '200px'
              : openFilter.includes('brand') && filters.brandFilter.length < 10
                ? 'fit-content'
                : '0px'
      ...
      <CategoriesContainer>
        ...
        <Categories
          height={
            openFilter.includes('size') && filters.sizeFilter.length >= 10
              ? '200px'
              : openFilter.includes('size') && filters.sizeFilter.length < 10
                ? 'fit-content'
                : '0px'
      ...
      <CategoriesContainer>
        ...
        <Categories
          height={
            openFilter.includes('color') && filters.sizeFilter.length >= 10
              ? '200px'
              : openFilter.includes('color') && filters.sizeFilter.length < 10
                ? 'fit-content'
                : '0px'
            ...
            <FilterColorContainer>
              {filters.colorFilter.map((item) => (
                <FilterColor
                  c={item}
                  key={item}
                  onClick={() => onToggleFilter('color', item)}
                  to={getFiltersUrl({ color: item })}
                >
                  {cat && Object.entries(cat).flat().includes(item) ? (
                    <StyledCheckbox
                      style={{
                        fontSize: '15px',
                        stroke: '#fff',
      ...
      <ApplyBtn onClick={() => setOpenFilters(false)}>Show Results</ApplyBtn>
    </FilterContainer>
  );
};
